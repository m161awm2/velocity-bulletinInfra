...
flowchart TB
    ...
    CF[CloudFront\napp.example.com]
    WAF[AWS WAF]
    FrontS3[(Private Frontend S3)]
    MediaCF[CloudFront\nmedia.example.com]
    MediaS3[(Private Media S3)]
    Quarantine[(Quarantine S3)]
    Validator[Image Validator Lambda]
    ...
    subgraph VPC[VPC - ap-northeast-2]
        subgraph AZA[AZ A]
            ...
            Mgmt[Private Management Subnet\nBreak-glass EC2: disabled]
        end
        ...
        OptionalEP[Optional Interface Endpoints\nECR / Logs / Secrets / SSM]
    end
    ...
    Engineer[Engineer SSO]

    User --> DNS --> CF
    WAF -. Web ACL association .-> CF
    CF -->|default /*| FrontS3
    CF -->|/api/* via VPC Origin| ALB
    ALB --> TaskA
    ALB --> TaskB

    User -->|presign 요청| CF
    User -->|presigned PUT| Quarantine
    Quarantine --> Validator --> MediaS3
    User --> MediaCF --> MediaS3

    TaskA -->|AZ-local egress| PubA --> Neon
    TaskB -->|AZ-local egress| PubB --> Neon
    TaskA --> S3EP
    TaskB --> S3EP
    TaskA -.-> OptionalEP
    TaskB -.-> OptionalEP
    ...
    GHA -->|push| ECR
    Engineer -->|pull| ECR
    ECR -->|Task Execution Role| TaskA
    ECR -->|Task Execution Role| TaskB
    Secrets --> TaskA
    Secrets --> TaskB
    TaskA --> Logs
    TaskB --> Logs
